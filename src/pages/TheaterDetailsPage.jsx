...
function RoomSlot({ slot, date, selected, onChoose, onSelect }) {
  ...
  return <button type="button" onClick={goToBooking} disabled={!isAvailable} className={`inline-flex min-h-[42px] w-auto min-w-[88px] flex-col items-center justify-center rounded-md border px-2 py-1 text-center text-[10px] leading-3 transition ${selected ? 'border-[#208653] bg-[#208653] text-white shadow-sm' : isBooked ? 'cursor-not-allowed border-[#c7c7c7] bg-[#e5e5e5] text-[#222222] opacity-75' : isChecking ? 'cursor-wait border-[#d9d9d9] bg-[#f7f7f7] text-[#222222]' : 'cursor-pointer border-[#c9c9c9] bg-white text-[#111111] hover:border-[#a9651c] hover:bg-[#fffaf5]'}`}><span className="block font-bold">{selected && '✓ '}{slot.startTime || slot.time?.split(' - ')[0]} -<br />{slot.endTime || slot.time?.split(' - ')[1]}</span><span className="mt-0.5 text-[8px] font-semibold uppercase tracking-wide">{selected ? 'Selected' : isBooked ? 'Sold out' : isChecking ? 'Checking' : date ? 'Available' : 'Select date'}</span></button>;
}
