...
export function ServicesPage() {
  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-5 max-w-[1440px] mx-auto">
      <h1 className="text-4xl font-heading font-black text-[#181533] mb-8 text-center">Our Services</h1>
      <p className="text-center text-gray-600 mb-12 max-w-2xl mx-auto">Discover the amazing services we offer for your celebrations.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <motion.div 
              ...
              key={service._id} 
              className="bg-white rounded-xl shadow-sm border border-[#f0e6dd] overflow-hidden hover:shadow-md transition-shadow"
            >
              <div 
              ...
              <div className="p-6">
                <h3 className="text-xl font-bold text-[#181533] mb-3">{service.title}</h3>
                <p className="text-gray-600 text-sm mb-4 leading-relaxed">{service.description}</p>
                {service.features && service.features.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-100">
                    {service.features.map((feature, i) => (
                      <span key={i} className="bg-[#FAF4ED] text-[#8c5211] text-xs font-medium px-2.5 py-1 rounded-full">
                        {feature}
                      </span>
                    ))}
                  </div>
